/* modules/rent.js — Rent & Tenant — openTenModal, saveTenant, payments, getTenantLedger, renderRent
...
Object.assign(APP, {
  _migrateRentHikeData(){
    if(this._rentHikeDataMigrated) return;
    this._rentHikeDataMigrated=true;
    const tenants=this.tenants||[];
    let changed=false;
    const cleaned=tenants.map(function(t){
      if(!t||typeof t!=='object') return t;
      const next={...t};
      if(Object.prototype.hasOwnProperty.call(next,'rentHistory')){
        delete next.rentHistory;
        changed=true;
      }
      if(Object.prototype.hasOwnProperty.call(next,'escalation')){
        delete next.escalation;
        changed=true;
      }
      return next;
    });
    if(changed) S.set('tenants', cleaned);
    try{ delete window._tnmRentHistory; }catch(e){}
  },

  _normalizeTenantContactList(values){
  ...
  _getTenantPhones(t){
    ...
    const fromArray = this._normalizeTenantContactList(t.phones);
    if(fromArray.length) return fromArray;
    return this._normalizeTenantContactList([t.ph, t.ph2]);
  },

  _getTenantEmails(t){
    if(!t || typeof t !== 'object') return [];
    const fromArray = this._normalizeTenantContactList(t.emails);
    if(fromArray.length) return fromArray;
    return this._normalizeTenantContactList([t.email, t.email2]);
  },

  _collectTenantContactValues(kind){
    const wrap = document.getElementById(kind === 'phone' ? 'tnm_phone_fields' : 'tnm_email_fields');
    if(!wrap) return [];
    return this._normalizeTenantContactList(this._collectTenantContactDraftValues(kind));
  },

  _collectTenantContactDraftValues(kind){
    const wrap = document.getElementById(kind === 'phone' ? 'tnm_phone_fields' : 'tnm_email_fields');
    if(!wrap) return [];
    return Array.from(wrap.querySelectorAll('input')).map(function(input){ return String(input.value || ''); });
  },

  _renderTenantContactFields(kind, values){
    const wrap = document.getElementById(kind === 'phone' ? 'tnm_phone_fields' : 'tnm_email_fields');
    if(!wrap) return;
    const raw = Array.isArray(values) ? values.map(function(item){ return String(item ?? ''); }) : [String(values ?? '')];
    const rows = raw.length ? raw : [''];
    const placeholderBase = kind === 'phone' ? '+91 XXXXXXXXXX' : 'tenant@email.com';
    const type = kind === 'email' ? 'email' : 'text';
    const inputmode = kind === 'phone' ? 'tel' : 'email';
    const labelBase = kind === 'phone' ? 'Mobile' : 'Email';
    wrap.innerHTML = rows.map(function(value, index){
      const n = index + 1;
      const placeholder = n === 1 ? placeholderBase : `${labelBase} ${n}`;
      const remove = index > 0
        ? `<button type="button" class="tenant-contact-remove" onclick="APP.removeTenantContactField('${kind}',${index})" aria-label="Remove ${labelBase.toLowerCase()} ${n}">×</button>`
        : '';
      return `<div class="tenant-contact-row">
        <input type="${type}" inputmode="${inputmode}" data-contact-kind="${kind}" data-contact-index="${index}" placeholder="${placeholder}" value="${String(value || '').replace(/"/g,'&quot;')}">
        ${remove}
      </div>`;
    }).join('');
  },

  addTenantContactField(kind, value=''){
    const values = this._collectTenantContactDraftValues(kind);
    values.push(value || '');
    this._renderTenantContactFields(kind, values);
    const wrap = document.getElementById(kind === 'phone' ? 'tnm_phone_fields' : 'tnm_email_fields');
    const last = wrap ? wrap.querySelector('.tenant-contact-row:last-child input') : null;
    if(last) last.focus();
  },

  removeTenantContactField(kind, index){
    const values = this._collectTenantContactDraftValues(kind);
  ...
  openTenModal(id,propId){
    ...
    if(id){
      ...
      ['name','idtype','idno','addr','rent','sec','adv','maint','mode','status','doc','notes'].forEach(f=>sv('tnm_'+f,t[f]||''));
      this._renderTenantContactFields('phone', this._getTenantPhones(t));
      this._renderTenantContactFields('email', this._getTenantEmails(t));
      ps.value=t.propId;
      ...
      ['name','idno','addr','rent','sec','adv','maint','doc','notes'].forEach(f=>sv('tnm_'+f,''));
      this._renderTenantContactFields('phone', ['']);
      this._renderTenantContactFields('email', ['']);
      document.getElementById('tnm_due').value='5';
      document.getElementById('tnm_idtype').value='';
      document.getElementById('tnm_mode').value='NEFT';
  ...
  saveTenant(){
    ...
    try {
      ...
      const phones = this._collectTenantContactValues('phone');
      const emails = this._collectTenantContactValues('email');
      const data={name,phones,emails,ph:phones[0]||'',ph2:phones[1]||'',email:emails[0]||'',email2:emails[1]||'',idtype:v('tnm_idtype'),idno:v('tnm_idno'),addr:v('tnm_addr'),propId:v('tnm_prop'),rent:Number(rent),sec:Number(v('tnm_sec')),adv:Number(v('tnm_adv')),maint:Number(v('tnm_maint'))||0,start:vDate('tnm_start'),end:vDate('tnm_end'),due:Number(v('tnm_due')),mode:v('tnm_mode'),status:v('tnm_status'),doc:v('tnm_doc'),notes:v('tnm_notes'),recurring:v('tnm_recurring'),invdate:vDate('tnm_invdate'),docFiles:_finalDocFiles};
  ...
  openPayModal(tid){
    ...
    try{sv('pym_tds_amt','0');var _tp=document.getElementById('pym_tds_preview');if(_tp)_tp.style.display='none';}catch(_e){}
  ...
  openEditPayModal(pid){
    ...
    sv('pym_ref',p.ref||'');
    sv('pym_note',p.note||'');
    try{sv('pym_tds_amt',p.tdsAmt||'0');}catch(_e){}
  ...
  savePayment(){
    return this._runGuardedAction('savePayment', (release)=>{
    ...
    const _restoreBtn = () => {
      if(_saveBtn){
        ...
        _saveBtn.textContent = ptype2 === 'refund' ? 'Save Refund' : 'Save Payment';
      }
    };
    ...
    try {
      ...
      const bankAmt  = parseFloat((v('pym_amt')||'').replace(/,/g,''))||0;
      ...
      if(this.payEditId){
        ...
        if(!existing){ alert('Error: Payment record not found!'); _restoreBtn(); release(); return; }
        ...
        this.showToastMsg(ptype==='refund' ? '↩️ Refund recorded!' : '💰 Payment of ₹'+Number(bankAmt).toLocaleString('en-IN')+' saved!');
      }
    ...
    <div class="receipt">
      ...
      <div class="section">
        ...
        <div class="grid2">
          ...
          <div class="field"><label>Payment Mode</label><span>${p.mode||'Cash'}</span></div>
      ...
      <div class="amount-box">
        <div class="amount-label">Total Rent Credited</div>
        <div class="amount-value">₹${Number((p.bankAmt||p.amount||0)+(p.tdsAmt||0)).toLocaleString('en-IN')}</div>
        <div class="amount-words">${(function(){
          const n=Math.floor(Number((p.bankAmt||p.amount||0)+(p.tdsAmt||0)));
        ...
        ${Number(p.tdsAmt||0)>0?`<div style="margin-top:8px;font-size:12px;color:#5b6478;">In Bank: <b>${fmt(Number(p.bankAmt||p.amount||0))}</b> · TDS Deducted: <b>${fmt(Number(p.tdsAmt||0))}</b></div>`:''}
      </div>
  ...
  setRentSub(s){
    ...
    this.rentSub=allowed.includes(s)?s:'overview';
    if(this.curTab==='rent' && this.syncCurrentRoute) this.syncCurrentRoute();
    this.renderRent();
  },

  _updateTdsPreview(){
    try{
      var bank=parseFloat((document.getElementById('pym_amt')||{}).value||0)||0;
      var tds=parseFloat((document.getElementById('pym_tds_amt')||{}).value||0)||0;
      var prev=document.getElementById('pym_tds_preview');
      if(!prev) return;
      if(tds>0){
        prev.style.display='block';
        prev.innerHTML='In Bank: ₹'+bank.toLocaleString('en-IN')+' + TDS Deducted: ₹'+tds.toLocaleString('en-IN')+' = Total Rent Credited: ₹'+(bank+tds).toLocaleString('en-IN');
  ...
  getTenantLedger(t){
    ...
    const ledger=months.map(({year,month})=>{
      ...
      const bankReceived=monthPays.reduce((s,p)=>p.ptype==='refund'?s-Number(p.bankAmt||p.amount):s+Number(p.bankAmt||p.amount),0);
      const tdsReceived=monthPays.reduce((s,p)=>p.ptype==='refund'?s:s+Number(p.tdsAmt||0),0);
      const received=bankReceived+tdsReceived;
      ...
      return{
        year,month,monthlyTotal,received,bankReceived,tdsReceived,
  ...
  renderRent(){
    ...
    try{ this.syncRentReminders(); }catch(e){}
    const allowedTabs=['overview','tenants','ledger','history','templates'];
    const s=allowedTabs.includes(this.rentSub)?this.rentSub:'overview';
    if(this.rentSub!==s) this.rentSub=s;
    const now=new Date(),m=now.getMonth(),y=now.getFullYear();
    const tabs=[['overview','📊 Overview'],['tenants','👥 Tenants'],['ledger','📒 Full Ledger'],['history','📜 Payments'],['templates','✉️ Message Templates']];
    let html='';
    ...
    if(s==='overview'){
      ...
      let totExp=0,totRec=0,totBal=0,totBank=0,totTds=0;
      ...
      const rows=this.tenants.filter(t=>t.status==='active').map(t=>{
        const prop=this.props.find(p=>p.id===t.propId);
        const ledger=this.getTenantLedger(t);
        totExp+=ledger.totalExpected; totRec+=ledger.totalReceived; totBal+=ledger.totalBalance; totBank+=ledger.totalBank||0; totTds+=ledger.totalTds||0;
        const curMo=ledger.months.find(mo=>mo.year===y&&mo.month===m);
        ...
        return`<div class="rent-tenant-card ${isOvd?'is-overdue':''} ${curBal>0&&!isOvd?'has-balance':''}">
          <div class="rent-tenant-head">
            <div class="rent-tenant-identity">
              <div class="rent-tenant-avatar"><span class="material-symbols-outlined" aria-hidden="true">person</span></div>
              <div class="rent-tenant-copy">
                <div class="rent-tenant-name">${t.name}</div>
                <div class="rent-tenant-line rent-tenant-line--icon"><span class="material-symbols-outlined" aria-hidden="true">location_on</span><span>${prop?prop.name:'\u2014'}</span></div>
                <div class="rent-tenant-line rent-tenant-line--icon"><span class="material-symbols-outlined" aria-hidden="true">call</span><span>${t.ph||'No phone'}</span></div>
                <div class="rent-tenant-line rent-tenant-line--meta">Invoice: ${t.invdate?new Date(t.invdate).getDate():1}st · Due: ${t.due}th · ${fmt(t.rent)}/mo</div>
                ${t.sec>0?`<div class="rent-tenant-line rent-tenant-line--icon rent-tenant-line--security"><span class="material-symbols-outlined" aria-hidden="true">lock</span><span>Security: ${fmt(t.sec)}</span></div>`:''}
              </div>
            </div>
            <div class="rent-tenant-badges">
              <span class="rent-punctuality" style="background:${_punctBg};color:${_punctClr};">${_punctLabel}${_punctScore!==null?' '+_punctScore+'%':''}</span>
              <span class="rent-status-pill" style="background:${statusBg};color:${statusColor};">${statusLabel}</span>
              ${curMo&&curMo.status==='upcoming'?`<div class="rent-status-note is-upcoming">Invoice in ${curMo.daysUntilInv}d</div>`:''}
              ${isOvd?`<div class="rent-status-note is-overdue">${curMo?curMo.daysOverdue:0}d overdue</div>`:''}
            </div>
          </div>
          <div class="rent-tenant-stats">
            <div class="rent-tenant-stat">
              <div class="rent-tenant-stat-label">This Month</div>
              <div class="rent-tenant-stat-value ${curBal>0?'is-red':'is-green'}">${curMo&&curMo.status==='upcoming'?'—':(curBal>0?fmt(curBal):'\u2713 Paid')}</div>
            </div>
            <div class="rent-tenant-stat">
              <div class="rent-tenant-stat-label">Received</div>
              <div class="rent-tenant-stat-value is-green">${fmt(ledger.totalReceived)}</div>
            </div>
            <div class="rent-tenant-stat">
              <div class="rent-tenant-stat-label">Outstanding</div>
              <div class="rent-tenant-stat-value ${ledger.totalBalance>0?'is-red':'is-green'}">${ledger.totalBalance>0?fmt(ledger.totalBalance):'\u2713 Clear'}</div>
            </div>
          </div>
          <div class="rent-tenant-actions">
            <div class="rent-tenant-actions-left">
              ${curBal>0?`<button class="rent-action-btn rent-action-btn--soft-green" onclick="APP.openPayModal('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">payments</span><span>Pay</span></button>`:''}
              <button class="rent-action-btn" onclick="APP.openTenModal('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">edit</span><span>Edit</span></button>
              <button class="rent-action-btn" onclick="APP.goTab('rent');APP.viewLedgerTid='${t.id}';APP.setRentSub('ledger')"><span class="material-symbols-outlined" aria-hidden="true">account_balance_wallet</span><span>Ledger</span></button>
            </div>
            <div class="rent-tenant-actions-right">
              <div class="rent-split-action rent-split-action--wa">
                <div class="rent-split-action-title"><span class="material-symbols-outlined" aria-hidden="true">chat</span><span>WhatsApp</span></div>
                <div class="rent-split-action-links">
                  ${t.ph
                    ? `<a href="https://wa.me/${t.ph.replace(/\D/g,'')}?text=${quickWaMsg}" target="_blank" class="rent-split-link">Quick</a><a href="https://wa.me/${t.ph.replace(/\D/g,'')}?text=${waRentMsg}" target="_blank" class="rent-split-link">Template</a>`
                    : `<button class="rent-split-single is-muted" onclick="APP._tenWA('${t.id}')">No Phone — Add</button>`}
                </div>
              </div>
              <div class="rent-split-action rent-split-action--email">
                <div class="rent-split-action-title"><span class="material-symbols-outlined" aria-hidden="true">mail</span><span>Email</span></div>
                <div class="rent-split-action-links">
                  ${t.email
                    ? `<a href="mailto:${t.email}?subject=${quickMailSubj}&body=${quickMailBody}" class="rent-split-link">Quick</a><a href="mailto:${t.email}?subject=${mailRentSubj}&body=${mailRentBody}" class="rent-split-link">Template</a>`
                    : `<button class="rent-split-single is-muted" onclick="APP._tenEmail('${t.id}')">No Email — Add</button>`}
                </div>
              </div>
              <button class="rent-action-btn" onclick="APP._showTenDocs('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">description</span><span>Files${t.docFiles&&t.docFiles.length?' ('+t.docFiles.length+')':''}</span></button>
            </div>
          </div>
        </div>`;
      }).join('');
      ...
      html=`
        <div class="rent-overview-kpis">
          <div class="rent-overview-kpi">
            <div class="rent-overview-kpi-label">Total Expected</div>
            <div class="rent-overview-kpi-value">${fmt(totExp)}</div>
          </div>
          <div class="rent-overview-kpi is-green">
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;align-items:stretch;height:100%;">
              <div style="background:rgba(255,255,255,.72);border:1px solid rgba(34,197,94,.18);border-radius:12px;padding:10px 12px;">
                <div style="font-size:.68rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--mut);margin-bottom:4px;">Total Received</div>
                <div style="font-size:1rem;font-weight:800;color:var(--grn);font-family:'JetBrains Mono',monospace;">${fmt(totRec)}</div>
              </div>
              <div style="background:rgba(255,255,255,.72);border:1px solid rgba(34,197,94,.18);border-radius:12px;padding:10px 12px;">
                <div style="font-size:.68rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--mut);margin-bottom:4px;">In Bank</div>
                <div style="font-size:1rem;font-weight:800;color:var(--grn);font-family:'JetBrains Mono',monospace;">${fmt(totBank)}</div>
              </div>
              <div style="background:rgba(255,255,255,.72);border:1px solid rgba(124,58,237,.16);border-radius:12px;padding:10px 12px;">
                <div style="font-size:.68rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--mut);margin-bottom:4px;">TDS</div>
                <div style="font-size:1rem;font-weight:800;color:#7c3aed;font-family:'JetBrains Mono',monospace;">${fmt(totTds)}</div>
              </div>
            </div>
          </div>
          <div class="rent-overview-kpi ${totBal>0?'is-red':'is-green'}">
            <div class="rent-overview-kpi-label">Total Outstanding</div>
            <div class="rent-overview-kpi-value">${totBal>0?fmt(totBal):'✅ Clear'}</div>
            <div class="rent-overview-kpi-copy">Rent only</div>
          </div>
        </div>
        ${rows?`<div class="rent-overview-heading">🏠 Rent Outstanding</div>`:''}
        <div class="rent-overview-list">${rows||'<div class="empty"><div class="ei">👥</div>No active tenants</div>'}</div>`;
    }
    if(s==='tenants'){
      const cards=this.tenants.map(t=>{
        ...
        return`<div class="card">
          ...
          <div class="card-foot rent-tenant-card-actions">
            <button class="rent-action-btn rent-action-btn--soft-green" onclick="APP.openPayModal('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">payments</span><span>Payment</span></button>
            <button class="rent-action-btn rent-action-btn--soft-red" onclick="APP._openRefundModal('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">reply</span><span>Refund</span></button>
            <button class="rent-action-btn" onclick="APP.goTab('rent');APP.viewLedgerTid='${t.id}';APP.setRentSub('ledger')"><span class="material-symbols-outlined" aria-hidden="true">account_balance_wallet</span><span>Ledger</span></button>
            <button class="rent-action-btn" onclick="APP.openTenModal('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">edit</span><span>Edit</span></button>
            <button class="rent-action-btn rent-action-btn--soft-green" onclick="APP._tenWA('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">chat</span><span>WhatsApp</span></button>
            <button class="rent-action-btn rent-action-btn--soft-blue" onclick="APP._tenEmail('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">mail</span><span>Email</span></button>
            <button class="rent-action-btn" onclick="APP._showTenDocs('${t.id}')"><span class="material-symbols-outlined" aria-hidden="true">description</span><span>Files${t.docFiles&&t.docFiles.length?' ('+t.docFiles.length+')':''}</span></button>
          </div>
        </div>`;
      }).join('');
    ...
    if(s==='ledger'){
          ...
          <div class="date-filter-bar">
            <div class="date-filter-bar__tools">
              ${renderCompactDateRangeFilter({
                ...
                fromId:'dfh_rlf',
                toId:'dfh_rlt',
                fromValue:this._ledgerCustomFrom||'',
                toValue:this._ledgerCustomTo||'',
                fromOnChange:"APP._ledgerFilter='custom';APP._ledgerCustomFrom=this.value;APP.renderRent()",
                toOnChange:"APP._ledgerFilter='custom';APP._ledgerCustomTo=this.value;APP.renderRent()",
                clearOnClick:"APP._ledgerFilter='all';APP._ledgerCustomFrom='';APP._ledgerCustomTo='';APP.renderRent()",
      ...
      if(selTenId==='all'){
        ...
        // Summary stats
        const totalPay = allPays.filter(p=>p.ptype!=='refund').reduce((s,p)=>s+Number(p.bankAmt||p.amount||0)+Number(p.tdsAmt||0),0);
        const totalRef = allPays.filter(p=>p.ptype==='refund').reduce((s,p)=>s+Number(p.bankAmt||p.amount||0),0);
        ...
        const rows = allPays.map(p=>{
          ...
          return`<tr style="${isRef?'background:#fff9f9':''}">
            ...
            <td class="mono tr" style="color:${col};font-weight:700;">${sign}${fmt(Number(p.bankAmt||p.amount||0))}</td>
            <td class="mono tr" style="color:#7c3aed;font-weight:700;">${Number(p.tdsAmt||0)>0?fmt(Number(p.tdsAmt||0)):'—'}</td>
        ...
        html = `${filterBar}${tenSel}
          <div class="export-toolbar">
            ${APP._pdfOriHtml()}
            <button onclick="APP._downloadLedgerPDF('all')" class="btn b-out export-tool-btn export-tool-pdf"><span class="material-symbols-outlined">picture_as_pdf</span><span>PDF</span></button>
            <button onclick="APP._downloadLedgerWord('all')" class="btn b-out export-tool-btn export-tool-word"><span class="material-symbols-outlined">description</span><span>Word</span></button>
            <button onclick="APP._downloadLedgerCSV('all')" class="btn b-out export-tool-btn export-tool-csv"><span class="material-symbols-outlined">table_view</span><span>CSV</span></button>
          </div>
          ...
          <div class="tbl-wrap"><table>
            <thead><tr><th>Date</th><th>Tenant</th><th>Property</th><th>Type</th><th class="tr">In Bank</th><th class="tr">TDS Deducted</th><th>Details</th><th>Edit</th></tr></thead>
            <tbody>${rows||'<tr><td colspan="8"><div class="empty">No transactions found</div></td></tr>'}</tbody>
          </table></div>`;
        ...
        if(!selTen){ html=tenSel+'<div class="empty"><div class="ei">👥</div>Select a tenant</div>'; }
          ...
          filteredMonths.slice().reverse().forEach(mo=>{
            ...
            allPayRows.push(`<tr style="border-top:2px solid var(--bdr2);${mRowBg}">
              ...
              <td class="mono" style="font-size:.72rem;">${mo.bankReceived?'<div style="color:var(--grn);font-weight:700;">'+fmt(mo.bankReceived)+'</div>':'<span style="color:var(--mut)">—</span>'}</td>
              <td class="mono" style="font-size:.72rem;">${mo.tdsReceived?'<div style="color:#7c3aed;font-weight:700;">'+fmt(mo.tdsReceived)+'</div>':'<span style="color:var(--mut)">—</span>'}</td>
            ...
            if(mo.payments.length===0 && mo.charged){
              allPayRows.push(`<tr style="${mRowBg}">
                <td colspan="9" style="padding:4px 14px;font-size:.72rem;color:var(--mut);font-style:italic;">No payments recorded for this month</td>
              </tr>`);
            }
            mo.payments.forEach(p=>{
              const isRef=p.ptype==='refund';
              ...
              allPayRows.push(`<tr style="background:${bg};">
                ...
                <td></td>
                <td class="mono" style="font-size:.72rem;">${(function(){var b=Number(p.bankAmt||p.amount);return isRef?'<div style="color:var(--red);font-weight:700">− '+fmt(b)+'</div>':'<div style="color:var(--grn);font-weight:700">'+fmt(b)+'</div>';})()}</td>
                <td class="mono" style="font-size:.72rem;">${(function(){var t=Number(p.tdsAmt||0);return t>0?'<div style="color:#7c3aed;font-weight:700">'+fmt(t)+'</div>':'<span style="color:var(--mut)">—</span>';})()}</td>
          ...
          const tenDlBtns = `<div class="export-toolbar">
            ${APP._pdfOriHtml()}
            <button onclick="APP._downloadLedgerPDF('${selTen.id}')" class="btn b-out export-tool-btn export-tool-pdf"><span class="material-symbols-outlined">picture_as_pdf</span><span>PDF</span></button>
            <button onclick="APP._downloadLedgerWord('${selTen.id}')" class="btn b-out export-tool-btn export-tool-word"><span class="material-symbols-outlined">description</span><span>Word</span></button>
            <button onclick="APP._downloadLedgerCSV('${selTen.id}')" class="btn b-out export-tool-btn export-tool-csv"><span class="material-symbols-outlined">table_view</span><span>CSV</span></button>
          </div>`;

          html=`${filterBar}${tenSel}${tenDlBtns}
              ...
              <div class="card-body">
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;">
                  <div style="background:var(--dim);border-radius:8px;padding:8px 11px;"><div class="pill-lbl">Property</div><div style="font-size:.8rem;font-weight:600">${prop?prop.name:'—'}</div></div>
	                  ...
	                  ${ledger.totalTds>0?'<div style="background:#f5f3ff;border-radius:8px;padding:8px 11px;"><div class="pill-lbl" style="color:#7c3aed">TDS Deducted</div><div style="font-size:.8rem;font-weight:700;color:#7c3aed">'+fmt(ledger.totalTds)+'</div></div>':''}
            ...
            <div class="tbl-wrap"><table>
              <thead><tr><th>Month / Entry</th><th>Invoice Date</th><th>Rent Month / Pay Date</th><th>Charged</th><th>In Bank</th><th>TDS</th><th>Balance / Mode</th><th>Status</th><th>Action</th></tr></thead>
              <tbody>${allPayRows.join('')||'<tr><td colspan="9"><div class="empty">No data in range</div></td></tr>'}</tbody>
            </table></div>`;
        }
      }
    }
    ...
    if(s==='history'){
      ...
      const histTotal=histPays.filter(p=>p.ptype!=='refund').reduce((s,p)=>s+Number(p.bankAmt||p.amount)+Number(p.tdsAmt||0),0);
      const rows=histPays.map(p=>{
        ...
        return`<tr>
          ...
          <td>${t?t.name:'—'}</td>
          <td class="mono tr" style="color:${p.ptype==='refund'?'var(--red)':'var(--grn)'};font-weight:700">${p.ptype==='refund'?'− ':''}${fmt(bankAmt)}</td>
          <td class="mono tr" style="color:#7c3aed;font-weight:700">${tdsAmt>0?fmt(tdsAmt):'—'}</td>
          <td>${p.mode||'—'}</td>
      ...
      html=`
          ...
          <div class="date-filter-bar">
            <div class="date-filter-bar__tools">
              ${renderCompactDateRangeFilter({
                ...
                fromValue:histFrom,
                toValue:histTo,
                fromOnChange:"APP._histFrom=this.value;APP.renderRent()",
                toOnChange:"APP._histTo=this.value;APP.renderRent()",
                clearOnClick:"APP._histFrom='';APP._histTo='';APP.renderRent()",
                meta:`${histPays.length} entries · ${fmt(histTotal)}`,
                className:'date-filter-inline--tight'
              })}
            </div>
          </div>
        ...
        <div class="tbl-wrap"><table><thead><tr><th>Date</th><th>Property</th><th>Tenant</th><th class="tr">In Bank</th><th class="tr">TDS Deducted</th><th>Mode</th><th>Note</th><th></th></tr></thead>
        <tbody>${rows||'<tr><td colspan="8"><div class="empty">No payments for selected range</div></td></tr>'}</tbody></table></div>`;
    }
    ...
    document.getElementById('pan-rent').innerHTML=`<div class="stabs rent-tabs">${tabs.map(([k,l])=>`<button class="stab ${s===k?'on':''}" onclick="APP.setRentSub('${k}')">${l}</button>`).join('')}</div>${html}`;
  },
