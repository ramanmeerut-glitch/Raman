...
Object.assign(APP, {
      ...
      const detailHtml=`
        <div id="${detailId}" style="display:none;margin-top:10px;padding-top:10px;border-top:1px solid var(--bdr);font-size:.73rem;color:var(--mut);">
          ...
          ${!isRecurring&&!isRentAuto&&!isLoanAuto&&!isMedical?(function(){
            ...
            if(!_bd||isNaN(_bd)){var _rb=parseInt(e.before||0);_bd=_rb>1440?Math.round(_rb/1440):_rb;}
            var _due = e._dueDate||e.taskDate||e.dueDate||e.exp||'';
            ...
            if(_same){
              ...
              _h+='<span style="font-size:.62rem;color:var(--mut);min-width:76px;">🔔 Reminder Date</span>';
              ...
              _h+=(_bd>0?'<span style="font-size:.62rem;color:#6c757d;">('+_bd+' day'+(_bd>1?'s':'')+' before due date)</span>':'');
              _h+='</div>';
            }
            return _h;
          })():''}
        ...
        <div style="display:flex;align-items:center;gap:9px;">
          ...
          <div style="flex:1;min-width:0;">
            ...
            ${!isRentAuto&&!isRecurring?(function(){
              // dueDate = actual expiry/task date; alertDate = reminder trigger date
              var _due   = e._dueDate||e.taskDate||e.dueDate||e.exp||'';
              var _alert = e._alertDate||e.alertDate||e._trig||'';
              var _bd    = parseInt(e.beforeDays||0);
              if(!_bd||isNaN(_bd)){var _rb=parseInt(e.before||0);_bd=_rb>1440?Math.round(_rb/1440):_rb;}
              var _hasDue=_due&&_due.length===10;
              var _hasAlt=_alert&&_alert.length===10;
              var _same  =!_due||!_alert||_due===_alert||_bd===0;
              var _h='';
              // Due Date — always shown
              _h+='<div style="font-size:.63rem;font-weight:800;color:var(--txt);margin-top:3px;">📅 <span style="font-size:.58rem;color:var(--mut);font-weight:600;">Due Date:</span> '+(_hasDue?fD(_due):'—')+'</div>';
              // Reminder Date — always shown, never hidden
              if(_same){
                _h+='<div style="font-size:.62rem;color:#6c757d;margin-top:1px;">🔔 <span style="font-size:.58rem;color:var(--mut);">Reminder Date:</span> Same day as due date</div>';
              } else if(_hasAlt){
                _h+='<div style="font-size:.62rem;color:#1565c0;font-weight:700;margin-top:1px;">🔔 <span style="font-size:.58rem;color:#1565c0;">Reminder Date:</span> '+fD(_alert)+' <span style="font-size:.58rem;font-weight:400;color:#6c757d;">('+_bd+' day'+(_bd>1?'s':'')+' before due date)</span></div>';
              } else {
                _h+='<div style="font-size:.62rem;color:#6c757d;margin-top:1px;">🔔 <span style="font-size:.58rem;color:var(--mut);">Reminder Date:</span> —</div>';
              }
              return _h;
            })():''}
  ...
  _remBulkDoneAll(){
    ...
    const urgent = this.reminders.filter(r => {
      ...
      const trig = r.alertDate || r.taskDate || r.trigDate || r.exp || r.nextTrigger;
  ...
  _showAllRemindersPopup(){
    ...
    const getStatus = (r)=>{
      ...
      const trig = r.alertDate||r.taskDate||r.trigDate||r.reminderDate||r.exp||r.nextTrigger||'';
  ...
  _remDownloadPDF(filter){
    ...
    const rows = sorted.map(r=>{
      ...
      const amt  = isRent&&r._balanceAmt?'Rs.'+Number(r._balanceAmt).toLocaleString('en-IN'):'—';
      ...
      var _pDue  = isRent?date:fD(r.taskDate||r.dueDate||r.exp||'');
      var _pAlt  = isRent?date:fD(r.alertDate||r.taskDate||r.dueDate||r.exp||'');
  ...
  _remDownloadWord(){
    ...
    const sorted = [...rems].sort((a,b)=>{
      ...
      const ta=a.alertDate||a.taskDate||a.trigDate||a.exp||a.nextTrigger||'9999';
      const tb=b.alertDate||b.taskDate||b.trigDate||b.exp||b.nextTrigger||'9999';
...
<div class="legend">Sort order: Overdue → Due Today → This Week → This Month → Upcoming → No Date → Completed</div>
  ...
  _remExportCSV(){
    ...
    const header = ['Sr.','Name','Person','Type','Reminder Date (DD/MM/YYYY)','Expiry Date','Status','Mode','Frequency','Notes'];
