...
Object.assign(APP, {
  ...
  openPayModal(tid){
    ...
    try{sv('pym_tds_amt','0');sv('pym_tds_ref','');var _ts=document.getElementById('pym_tds_status');if(_ts)_ts.value='pending';var _tp=document.getElementById('pym_tds_preview');if(_tp)_tp.style.display='none';var _td=document.getElementById('pym_tds_date');if(_td)_td.value='';}catch(_e){}
  ...
  openEditPayModal(pid){
    ...
    sv('pym_ref',p.ref||'');
    sv('pym_note',p.note||'');
    try{sv('pym_tds_amt',p.tdsAmt||'0');sv('pym_tds_ref',p.tdsRef||'');var _ts2=document.getElementById('pym_tds_status');if(_ts2)_ts2.value=p.tdsStatus||'pending';var _td2=document.getElementById('pym_tds_date');if(_td2)_td2.value=p.tdsDate||p.date||'';}catch(_e){}
  ...
  savePayment(){
    ...
    try {
      ...
      const payData = {
        amount:    Number(amt),
        bankAmt:   Number(amt),
        tdsAmt, tdsDate, tdsRef, tdsStatus,
        date, rentForMonth,
  ...
  _updateTdsPreview(){
    try{
      ...
      if(tds>0){
        ...
        prev.innerHTML='Bank: ₹'+bank.toLocaleString('en-IN')+' + TDS: ₹'+tds.toLocaleString('en-IN')+' = Total: ₹'+(bank+tds).toLocaleString('en-IN');
        ...
        if(!selTen){ html=tenSel+'<div class="empty"><div class="ei">👥</div>Select a tenant</div>'; }
          ...
          filteredMonths.slice().reverse().forEach(mo=>{
            ...
            allPayRows.push(`<tr style="border-top:2px solid var(--bdr2);${mRowBg}">
              ...
              <td class="mono" style="font-size:.72rem;">${(function(){var b=mo.bankReceived||0,t=mo.tdsReceived||0;return (b>0?'<div style="color:var(--grn);font-weight:700;">Bank ₹'+fmt(b)+'</div>':'')+(t>0?'<div style="color:#7c3aed;font-weight:700;">TDS ₹'+fmt(t)+'</div>':'')+(b+t===0?'<span style="color:var(--mut)">—</span>':'');})()}</td>
            ...
            mo.payments.forEach(p=>{
              ...
              allPayRows.push(`<tr style="background:${bg};">
                ...
                <td></td>
                <td class="mono" style="font-size:.72rem;">${(function(){var b=Number(p.bankAmt||p.amount),t=Number(p.tdsAmt||0),st=p.tdsStatus==='verified'?'TDS✓':'TDS⏳';return isRef?'<div style="color:var(--red);font-weight:700">− ₹'+fmt(b)+'</div>':('<div style="color:var(--grn);font-weight:700">Bank ₹'+fmt(b)+'</div>'+(t>0?'<div style="color:#7c3aed;font-weight:700">'+st+' ₹'+fmt(t)+'</div>':''));})()}</td>
